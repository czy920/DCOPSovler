...
//import edu.cqu.algorithms.dcop.complete.Pruning;
...
public class HSCAI_BIP extends BIP {
    ...
    public HSCAI_BIP(int id, int[] domain, int[] neighbours, Map<Integer, int[][]> constraintCosts, Map<Integer, int[]> neighbourDomains, SyncMailer mailer,int Lastid) {
    ...
    public void disposeMessage(Message message) {
        ...
        switch (message.getType()) {
            ...
            case MSG_CPA:{
                ...
                msgSizeCnt += cpaMsg.cpa.size()*2 + 1+cpaMsg.DelValFromP.size();
                msgSizeCntSearchPart += cpaMsg.cpa.size()*2 + 1+cpaMsg.DelValFromP.size();
                ...
                DV_P.clear();
                DV_P.addAll(new HashSet<>(cpaMsg.DelValFromP));
                ...
                if (isLeafAgent()) {
                    ...
                    for (int i = 0; i < domain.length; i++) data[i] = localCost[i];
                    getDeletedValues(cpa,data);
                    getDomain();
...
//        System.out.println(" since " + str+"\n");
    ...
    void sendCpa(int child, int val) {
        ...
        sendMessage(new Message(id, child, MSG_CPA, new CpaMsg(newub,newCpa,collected,new HashSet<>(DeleteValueToAC[val].get(child)))));
    }
    ...
    private class CpaMsg{
        ...
        public CpaMsg(int ub, Map<Integer, Integer> cpa, boolean haveBeenRuled,Set<Integer> DelValFromP) {
            this.ub = ub;
